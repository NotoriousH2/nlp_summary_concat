...
def tokdemo(x,y):
  stories = os.listdir(x)
  # make IO list file
  print "Making list of files to tokenize..."
  with open("mapping.txt", "w") as f:
    for s in stories:
      f.write("%s \t %s\n" % (os.path.join(x, s), os.path.join(y, s)))
  command = ['java', 'edu.stanford.nlp.process.PTBTokenizer', '-ioFileList', '-preserveLines', 'mapping.txt']
  print "Tokenizing %i files in %s and saving in %s..." % (len(stories), x, y)
  subprocess.call(command)
  print "Stanford CoreNLP Tokenizer has finished."
  os.remove("mapping.txt")

  # Check that the tokenized stories directory contains the same number of files as the original directory
  num_orig = len(os.listdir(x))
  num_tokenized = len(os.listdir(y))
  if num_orig != num_tokenized:
    raise Exception("The tokenized stories directory %s contains %i files, but it should contain the same number as %s (which has %i files). Was there an error during tokenization?" % (tokenized_stories_dir, num_tokenized, stories_dir, num_orig))
  print "Successfully finished tokenizing %s to %s.\n" % (x, y)
...
def write_to_bin(url_file, out_file, makevocab=False):
  ...
  if makevocab:
    ...
    with open(os.path.join(finished_files_dir, "vocab"), 'a') as writer:
...
if __name__ == '__main__':
 # if len(sys.argv) != 2:
 #   print("USAGE: python preprocessing.py <data>")
 #   sys.exit()
 # cnn_stories_dir = sys.argv[1]
 # dm_stories_dir = sys.argv[2]
#  f=open("./vocab.old",'r');
#  words=collection.Counter();  
#  while True:
#    try:
#      w,c=f.readline().replace('\n','').split(',');
#      words.update({w:int(c)});
#    except:
#      break;
  tokdemo('./in','./out');
...
# write_to_bin(all_train_urls, os.path.join(finished_files_dir, "train.bin"), makevocab=True)
  f2=open("./chunked/test_001.bin",'w');
  f2.write("abstract\n<s></s>\n\narticle\n");
  f3=open("./in/1.txt",'r');  
  count=0;
  while True:
